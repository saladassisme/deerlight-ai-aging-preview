import {
  ...
  HeartPulse, Home, KeyRound, Link2, Search, ShieldCheck, Sparkles, Store, Users, X,
} from 'lucide-react'
...
const copy = {
  zh: {
    ...
    galleryTitle: '技能商城',
    galleryLead: '点击任意技能即可查看能力、对话开场白、精选案例和适用环境，无需跳转子页面。',
    search: '搜索技能或生活场景', noResult: '没有找到匹配的技能。',
    add: '添加技能', added: '已添加', paid: '付费', free: '免费',
    creator: '创建者', worksOn: '可用于', capabilities: '主要能力', starters: '对话开场白', cases: '精选案例',
    input: '用户输入', output: '生成结果', close: '关闭', details: '查看详情',
  ...
  en: {
    ...
    galleryLead: 'Open any skill in a modal to review capabilities, conversation starters, examples, and compatible environments.',
    search: 'Search skills or situations', noResult: 'No matching skill.',
    add: 'Add skill', added: 'Added', paid: 'Paid', free: 'Free',
    creator: 'Creator', worksOn: 'Works on', capabilities: 'Capabilities', starters: 'Conversation starters', cases: 'Featured examples',
    input: 'User input', output: 'Generated result', close: 'Close', details: 'View details',
...
const providerNames: Record<string, { zh: string; en: string }> = {
  ...
  Wayhome: { zh: '安心出行', en: 'Wayhome' }, Plainly: { zh: '明白说', en: 'Plainly' },
  'Deerlight Labs': { zh: '小鹿实验室', en: 'Deerlight Labs' }, Dayline: { zh: '日程线', en: 'Dayline' },
}

const richDetails: Record<number, {
  ...
  1: {
    ...
    example: { zh: ['“今晚前点击链接完成验证，否则账户冻结。”', '高风险：包含外部链接、强烈催促与账户威胁。请勿点击，改用官方应用核实。'], en: ['“Verify through this link tonight or your account will be frozen.”', 'High risk: external link, urgency, and account threat. Do not click; verify in the official app.'] },
  },
  3: {
    ...
    example: { zh: ['“夜间咳嗽两周，走快会喘，正在服用降压药。”', '就诊摘要：症状时间线、当前用药、需要询问的检查项目和需要立即就医的警示表现。'], en: ['“Night cough for two weeks, breathless when walking fast, taking blood-pressure medicine.”', 'Visit brief: symptom timeline, current medicine, questions about tests, and warning signs requiring urgent care.'] },
  },
  8: {
...
export default function SkillHubPage({ lang, setLang }: { lang: Lang; setLang: (lang: Lang) => void }) {
  ...
  return <main className="product-page skillhub-page">
    ...
    <section className="gallery-section">
      ...
      {filtered.length > 0 ? <div className="skill-gallery">{filtered.map((skill) => { const Icon = categoryIcons[skill.category]; const isAdded = added.includes(skill.id); return <article className={featuredIds.has(skill.id) ? 'featured' : ''} key={skill.id} onClick={() => setSelected(skill)}><div className="skill-card-top"><span className="skill-icon"><Icon size={21} /></span><span className="verified">{isFree(skill) ? t.free : t.paid}</span></div><h3>{tx(skill.name, lang)}</h3><p>{tx(skill.description, lang)}</p><div className="skill-provider"><span>{providerNames[skill.provider]?.[lang] ?? skill.provider}</span><strong>{tx(skill.price, lang)}</strong></div><button className="skill-card-action" onClick={(event) => { event.stopPropagation(); isAdded ? setSelected(skill) : addSkill(skill) }}>{isAdded ? <><Check size={16} />{t.added}</> : <>{t.add}<ChevronRight size={16} /></>}</button></article> })}</div> : <div className="empty-gallery"><Sparkles size={24} /><p>{t.noResult}</p></div>}
    </section>

    <section className="skill-delivery-section">
      <SectionTitle title={t.deliveryTitle} lead={t.deliveryLead} inverted />
      <div className="skill-delivery-grid">{t.delivery.map(([title, body], index) => { const Icon = index === 0 ? Home : index === 1 ? Code2 : Sparkles; return <article key={title}><Icon size={25} /><h3>{title}</h3><p>{body}</p></article> })}</div>
    </section>

    <section className="publisher-section">
      <div className="publisher-heading"><Store size={34} /><SectionTitle title={t.publishTitle} lead={t.publishLead} /></div>
      <div className="publisher-steps">{t.publisherSteps.map(([title, body], index) => <article key={title}><span>{String(index + 1).padStart(2, '0')}</span><h3>{title}</h3><p>{body}</p></article>)}</div>
      <button className="publisher-cta" onClick={() => alert(lang === 'zh' ? '发布者中心原型即将开放' : 'Publisher center prototype coming next')}>{t.openStore}<ChevronRight size={18} /></button>
    </section>

    <ContactBand lang={lang} title={lang === 'zh' ? '把真实能力带进技能中心' : 'Bring a real capability to SkillHub'} />
    <SiteFooter lang={lang} />

    {selected && <div className="market-modal-backdrop" onClick={() => setSelected(null)}><section className="market-modal" onClick={(event) => event.stopPropagation()}><button className="modal-close" onClick={() => setSelected(null)} aria-label={t.close}><X /></button><div className="modal-header"><span className="modal-icon">{(() => { const Icon = categoryIcons[selected.category]; return <Icon /> })()}</span><div><small>{providerNames[selected.provider]?.[lang] ?? selected.provider}</small><h2>{tx(selected.name, lang)}</h2><p>{tx(selected.description, lang)}</p></div></div><div className="modal-meta"><span><BadgeCheck size={15} />{t.creator}: {providerNames[selected.provider]?.[lang] ?? selected.provider}</span><span><Link2 size={15} />{t.worksOn}: SilverOS · CLI · VS Code</span></div><div className="modal-columns"><div><h3>{t.capabilities}</h3><ul>{(richDetails[selected.id]?.bullets[lang] ?? [tx(selected.description, lang), lang === 'zh' ? '提供清晰、可修改的结构化结果' : 'Produces clear, editable structured output', lang === 'zh' ? '关键操作保留用户确认' : 'Keeps user confirmation for consequential actions']).map((item) => <li key={item}>{item}</li>)}</ul><h3>{t.starters}</h3><div className="starter-list">{(richDetails[selected.id]?.starters[lang] ?? [lang === 'zh' ? `帮我使用${tx(selected.name, lang)}` : `Help me use ${tx(selected.name, lang)}`, lang === 'zh' ? '先告诉我需要准备什么' : 'Tell me what information you need']).map((item) => <button key={item}>{item}<ChevronRight size={15} /></button>)}</div></div><div><h3>{t.cases}</h3><div className="example-split"><div><span>{t.input}</span><p>{richDetails[selected.id]?.example[lang][0] ?? tx(selected.description, lang)}</p></div><div><span>{t.output}</span><p>{richDetails[selected.id]?.example[lang][1] ?? (lang === 'zh' ? '技能将根据输入生成结构化建议、解释和下一步操作。' : 'The skill produces structured guidance, explanation, and next actions.')}</p></div></div></div></div><footer><div><strong>{tx(selected.price, lang)}</strong><span>{isFree(selected) ? t.free : t.paid}</span></div><button onClick={() => addSkill(selected)}>{added.includes(selected.id) ? t.added : t.add}<ChevronRight size={17} /></button></footer></section></div>}
